...
import 'package:ecomapp/model/any_image.dart';
import 'package:ecomapp/model/category.dart';
import 'package:ecomapp/model/product.dart';
import 'package:ecomapp/util/remote_config.dart';
import 'package:ecomapp/widgets/products_list_item.dart';
...
class ProductsListPage extends StatelessWidget {
  ...
  _buildProductsListPage() {
    int defaultCategory = int.parse(RemoteConfig.config["DEFAULT_CATEGORY"]);
    Size screenSize = MediaQuery.of(context).size;
    return Container(
      color: Colors.grey[100],
      child: FutureBuilder<List<Product>>(
        future: _parseProductsFromResponse(defaultCategory),
  ...
  Future<dynamic> _getProductsByCategory(categoryId, pageIndex) async {
    var response = await http.get(
      RemoteConfig.config["BASE_URL"] +
          ...
          //"category=$categoryId + 
          "&consumer_key=" + RemoteConfig.config["ConsumerKey"] + 
          "&consumer_secret=" + RemoteConfig.config["ConsumerSecret"] + 
          "&per_page=6" + 
          "&page=$pageIndex",
      //headers: {
